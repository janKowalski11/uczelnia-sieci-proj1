...
def ping(ip): # arg is ip adress ex 192.168.102.2
    user_input=input("ping hosta (Y/N)")
    if(user_input.lower()=='y'):
        respons=os.system("ping "+ ip)


def ipBinaryToDecimal(ipBin):
    ipBin=ipBin.split(".")
    for idx, val in enumerate(ipBin):
        ipBin[idx] = str(int(val,2))
